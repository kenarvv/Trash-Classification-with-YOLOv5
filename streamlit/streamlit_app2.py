...
#     results = model(image)
#     results.save(save_dir=save_dir)  # Save detection results to directory
...
def main():
    ...
    image_files = sorted(glob.glob("test_images/*"))
    selected_image_path = st.selectbox("Choose an image to test:", image_files)

    # Tampilkan gambar yang dipilih
    if selected_image_path:
        st.image(selected_image_path, caption=f"Selected Image: {os.path.basename(selected_image_path)}", use_container_width=True)
    ...
    if st.button("Run Detection"):
        if os.path.exists(selected_model_path):
            ...
            exp_dirs = sorted(glob.glob(f"{result_dir}*"), key=os.path.getctime, reverse=True)
            if exp_dirs:
                latest_exp_dir = exp_dirs[0]
                result_images = glob.glob(f"{latest_exp_dir}/*.jpg")
                if result_images:
